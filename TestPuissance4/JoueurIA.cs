...
namespace TestPuissance4
{
    public class JoueurIA : Joueur
    {
        ...
        //private List<int> _colonnesToNotPlay = new List<int>();
        private int _niveau;
        ...
        //private List<int> ColonneToNotPlay { get => _colonnesToNotPlay; set => _colonnesToNotPlay = value; }
        ...
        public override void Jouer(Grille grille, Puissance4 jeu)
        {
            ...
            do
            {
                ...
                // 3 - Voir pour planifier sur deux coups
                ...
                else
                {
                    ...
                    // Niveaux :
                    // -1 - Algo de Mr.Chevalier
                    // 1 - [1-100 Hasard] X
                    // 2 - [1-50 WinOuContre] sinon [100 Hasard] X
                    // 3 - [1-80 WinOuContre] sinon [1-50 Jeu au milieu / 51-100 Hasard] X
                    // 4 - [1-100 WinOuContre] sinon [1-50 Jeu au milieu / 51-100 Jeu vers la moyenne]
                    // 5 - [1-100 WinOuContre] sinon [1-50 Prévision sur deux coups / 51-75 Jeu vers la moyenne / 76-100 Jeu au milieu]
                    // 6 - [1-100 WinOuContre] sinon [1-100 Prévision sur deux coups]
                    ...
                    if (!ColonnesToEventuallyPlay.Any())
                    {
                        ...
                        else if ((Niveau == 4 && (50 < StrategyDice && StrategyDice <= 100)) || (Niveau == 5 && (50 < StrategyDice && StrategyDice <= 75)))
                        {
                            ...
                            {
                                colonne = RandomSeed.Next(0, grille.Tableau.GetLength(0));
                                //int index = RandomSeed.Next(Availables.Count);
                                //colonne = Availables[index];
                            }
                ...
                else
                {
                    ...
                    else
                    {
                        do
                        {
                            //var printtest1 = this.NomJoueur;
                            //var printtest2 = this.ColonnesToEventuallyPlay;
                            colonne = RandomSeed.Next(0, grille.Tableau.GetLength(0));
                            ligne = grille.GetLigne(colonne);
                        } while (ligne < 0);
                        rester = false;
